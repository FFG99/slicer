...
TEST(DynamicModesTest, SmallGridProducesArtifact) {
  const fs::path path = fs::temp_directory_path() / "slicer_test_dynamic_modes.h5";

  slicer::DynamicModesParams params;
  ...
  auto artifact =
      slicer::ArtifactFile::create(path.string(), "dynamic_modes", "henon", job_params);

  slicer::DynamicModesCalculator calculator(params, "henon", SLICER_SYSTEMS_DIR);
