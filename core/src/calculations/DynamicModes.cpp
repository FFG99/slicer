...
DynamicModesCalculator::DynamicModesCalculator(DynamicModesParams params,
                                                 std::string system,
...
void DynamicModesCalculator::validate() const {
...
void DynamicModesCalculator::initialize_system() {
...
std::pair<size_t, std::vector<double>> DynamicModesCalculator::calculate_period(
  ...
  for (size_t i = 0; i < params_.num_iter_transient; ++i) {
    ...
    if (!is_finite_state(iteration_point)) {
      return {0, iteration_point};
    }
  }

  const std::vector<double> compare_point = iteration_point;
  const double accuracy_square = params_.accuracy * params_.accuracy;

  for (size_t step = 1; step <= params_.num_iter_attractor; ++step) {
    iteration_point = dynamical_system_->evaluate(iteration_point, thread_params);
    ...
    for (size_t i = 0; i < compare_point.size(); ++i) {
      const double delta = iteration_point[i] - compare_point[i];
      distance_square += delta * delta;
    }
    if (distance_square < accuracy_square) {
      return {step, iteration_point};
    }
  }

  return {params_.num_iter_attractor, iteration_point};
}

void DynamicModesCalculator::run(ArtifactFile& artifact) {
  const size_t grid_size = params_.steps;
  artifact.create_main_grid(grid_size, grid_size);
  artifact.write_frame(build_dynamic_modes_frame(params_, system_));
    ...
    for (size_t col_ind = 0; col_ind < grid_size; ++col_ind) {
      ...
      if (direction == "r") {
        for (size_t row_ind = 0; row_ind < grid_size; ++row_ind) {
          ...
          const auto [period, point] = calculate_period(curr_point, thread_params);
          row[row_ind] = period;
          curr_point = point;
...
#pragma omp critical
      {
        artifact.write_main_grid_row(current_col, row);
        progress.advance(1);
      }
    }
...
#pragma omp parallel for
    for (size_t row_ind = 0; row_ind < grid_size; ++row_ind) {
      ...
      std::vector<std::size_t> col_data(grid_size, 0);
...
#pragma omp critical
      {
        artifact.write_main_grid_col(current_row, col_data);
        progress.advance(1);
      }
    }
  } else if (direction == "n") {
#pragma omp parallel for
    for (size_t col_ind = 0; col_ind < grid_size; ++col_ind) {
      ...
      for (size_t row_ind = 0; row_ind < grid_size; ++row_ind) {
        ...
        thread_params[second_param_index_] =
            params_.second_param_min + col_ind * second_param_step;
        const auto [period, _] = calculate_period(params_.starting_point, thread_params);
        row[row_ind] = period;
      }

#pragma omp critical
      {
        artifact.write_main_grid_row(col_ind, row);
        progress.advance(1);
      }
    }
  } else {
